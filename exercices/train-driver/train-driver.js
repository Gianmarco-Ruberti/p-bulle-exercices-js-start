...
export function fixListOfWagons(ids) {
  ...
  const deuxprem = arr.slice(0,2);
  const rest = arr.slice(2);
  return rest.concat(deuxprem)
}
...
const idsArray = Array.from(ids);
  const wagonsToInsert = Array.from(missingWagons);

  const first = idsArray.slice(0, 1);

  const rest = idsArray.slice(1);

  return [...first, ...wagonsToInsert, ...rest];}
